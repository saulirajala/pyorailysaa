...
class Wind extends Component {
	...
	componentDidMount() {
		fetch( 'http://data.fmi.fi/fmi-apikey/0cafbc8d-c52c-4c09-aae0-2e15a22f7265/wfs?request=getFeature&storedquery_id=fmi::observations::weather::timevaluepair&place=palokka&parameters=temperature,windspeedms,winddirection&starttime=2018-05-01T11:00:00Z&timestep=15' )
			.then(
				( result ) => {
					result
						.text()
						.then( ( str ) => {
							...
							let results = parseString( str, function ( err, result ) {
								...
								let windDirection = results[2]['omso:PointTimeSeriesObservation'][0]['om:result'][0]['wml2:MeasurementTimeseries'][0]['wml2:point'][0]['wml2:MeasurementTVP'][0]['wml2:value'][0];
